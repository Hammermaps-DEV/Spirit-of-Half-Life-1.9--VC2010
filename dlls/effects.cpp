...
{
	if (!ShouldToggle(useType, static_cast<bool>(m_active)))
		return;
	if (m_active)
	{
		m_active = 0;
...
void CLaser::Use(CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value)
{
	...
	if (!ShouldToggle(useType, static_cast<bool>(active)))
		return;
...
{
	if (ShouldToggle(useType, static_cast<bool>(!(pev->spawnflags & SF_ENVMODEL_OFF))))
	{
		if (pev->spawnflags & SF_ENVMODEL_OFF)
...
{
	if (ShouldToggle(useType, static_cast<bool>(pev->body)))
	{
		pev->body = !pev->body;
	}
}
