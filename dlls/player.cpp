...
void LinkUserMessages(void)
{
	...
	gmsgAddShine = REG_USER_MSG("AddShine", -1);
...
int CBasePlayer::TakeDamage(entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType)
{
	...
	int ftrivial = (pev->health > 75 || m_lastDamageAmount < 5);
	int fmajor = (m_lastDamageAmount > 25);
	int fcritical = (pev->health < 30);
...
void CBasePlayer::PackDeadPlayerItems(void)
{
	...
	CWeaponBox *pWeaponBox = dynamic_cast<CWeaponBox*>(Create("weaponbox", pev->origin, pev->angles, edict()));
	...
	pWeaponBox->SetVelocity(pev->velocity * 1.2); // weaponbox has player's velocity, then some.
...
//				ALERT(at_console, "Removing %s. (id = %d)\n", pCurrentItem->m_pNext->pszName(), pCurrentItem->m_pNext->m_iId);
				pCurrentItem->m_pPlayer = NULL;
				pCurrentItem->m_pNext->Kill();
...
//				ALERT(at_console, "Keeping %s. (id = %d)\n", pCurrentItem->m_pNext->pszName(), pCurrentItem->m_pNext->m_iId);
		...
		if (!(1 << m_rgpPlayerItems[i]->m_iId & iWeaponMask))
		{
			...
			m_rgpPlayerItems[i]->m_pPlayer = NULL;
...
void CBasePlayer::Killed(entvars_t *pevAttacker, int iGib)
{
	...
	if (pev->velocity.x > 40 || pev->velocity.y > 40 || pev->velocity.z > 40) 
...
void CBasePlayer::PlayerDeathThink(void)
{
	if (FBitSet(pev->flags, FL_ONGROUND))
	{
		...
		if (flForward <= 0)
			...
			SetVelocity(flForward * pev->velocity.Normalize());
	}
...
void CBasePlayer::StartDeathCam(void)
{
	...
	CBaseEntity* pSpot = UTIL_FindEntityByClassname(NULL, "info_intermission");
	if (pSpot)
	{
		...
		while (iRand > 0)
		{
			CBaseEntity* pNewSpot = UTIL_FindEntityByTargetname(pSpot, "info_intermission");
		...
		edict_t *pTarget = FIND_ENTITY_BY_TARGETNAME(NULL, STRING(pSpot->pev->target));
		if (pTarget && !FNullEnt(pTarget))
		{
			// Calculate angles to look at camera target
			pev->angles = UTIL_VecToAngles(pTarget->v.origin - pSpot->pev->origin);
			pev->angles.x = -pev->angles.x;
		}
		else
		{
			pev->angles = pSpot->pev->v_angle;
		}
	}
	else
	{
...
void CBasePlayer::StartWelcomeCam(void)
{
	...
	edict_t* pSpot = FIND_ENTITY_BY_CLASSNAME(NULL, "info_intermission");
	if (!FNullEnt(pSpot))
	{
		// at least one intermission spot in the world.
		int iRand = RANDOM_LONG(0, 3);
...
void CBasePlayer::Jump()
{
	...
	{
		SetVelocity(pev->velocity + pev->basevelocity);
	}
}
...
void CBasePlayer::Spawn(void)
{
	...
	m_iFlashBattery = 99; //discharge flashlight at start
...
void CBasePlayer::EnableControl(BOOL fControl)
{
	...
	{
		pev->flags |= FL_FROZEN;
		SetVelocityZero(); //LRC - stop view bobbing
	}
	else
		pev->flags &= ~FL_FROZEN;
}
