...
public:
	...
	int	InSquad(void) const { return m_pSquadLeader != NULL; }
	int	SquadCount(void);
...
void CFlockingFlyer::SpreadFlock()
{
	...
	while (pList)
	{
		if (pList != this && (pev->origin - pList->pev->origin).Length() <= AFLOCK_TOO_CLOSE)
		{
			...
			pList->SetVelocity(pList->pev->velocity.Normalize());
			pList->SetVelocity((pList->pev->velocity + vecDir) * 0.5);
			pList->SetVelocity(pList->pev->velocity * flSpeed);
		}
...
void CFlockingFlyer::SpreadFlock2()
{
	...
	while (pList)
	{
		if (pList != this && (pev->origin - pList->pev->origin).Length() <= AFLOCK_TOO_CLOSE)
		{
			...
			SetVelocity(pev->velocity + vecDir);
		}
...
void CFlockingFlyer::FlockFollowerThink(void)
{
	...
	if (flDistToLeader > AFLOCK_TOO_FAR)
	{
		...
		SetVelocity((pev->velocity + vecDirToLeader) * 0.5);
	}
	...
	SetVelocity(pev->velocity * pev->speed);
